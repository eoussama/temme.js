"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
var TemmyError_1 = __importDefault(require("../models/TemmyError"));
var InvalidReferenceError = (function (_super) {
    __extends(InvalidReferenceError, _super);
    function InvalidReferenceError(reference) {
        var _this = _super.call(this, "") || this;
        _this.name = "InvalidReferenceError";
        _this.message = "A reference is invalid";
        _this.message = reference.length > 0 ? "\u201C" + reference + "\u201D is an invalid reference" : _this.message;
        return _this;
    }
    return InvalidReferenceError;
}(TemmyError_1.default));
exports.default = InvalidReferenceError;
//# sourceMappingURL=InvalidReferenceError.js.map