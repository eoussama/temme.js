...
exports.isValidHierarchy = isValidHierarchy;
...
function isValidHTMLElement(target) {
    return target != null && target instanceof HTMLElement;
}
function validateReference(hierarchy, references) {
    return references.filter((ref) => ref.hierarchy.ref === hierarchy.from.ref
        && (0, idfier_1.getTemmeId)(hierarchy) !== (0, idfier_1.getTemmeId)(ref.hierarchy))[0] != null;
}
function validateOptions(hierarchy) {
    for (const option in hierarchy) {
        const matchingOption = options_1.options.filter((opt) => opt.label === option)[0];
        const optionValue = hierarchy[option];
...
function validateTemplates(template) {
    ...
    for (const option in template) {
        if (options_1.forbiddenOptions.includes(option)) {
...
function validateReferences(hierarchy, references) {
    ...
    if (hierarchy.from.ref !== undefined) {
        const fromRef = hierarchy.from.ref;
        if (fromRef[0] === "@") {
            const selector = fromRef.substring(1);
            ...
            if (validateReference(hierarchy, references) === false && fromRef !== "") {
                throw new InvalidReferenceError_1.default(fromRef);
            }
        }
    }
    ...
    if (templates.length > 0) {
        templates.forEach((template) => {
            if (template.from.ref.length > 0) {
                const referencedElement = references.filter((ref) => ref.hierarchy.ref === template.from.ref)[0];
                const isTemplateRef = (0, idfier_1.getTemmeId)(referencedElement.hierarchy).length === 4;
...
function validateSubOptions(optionName, optionValue, subOptions) {
    for (const subOption in optionValue) {
        const matchingSubOption = (0, options_1.getSubOptions)(optionName).filter((sub) => sub.label === subOption)[0];
        ...
        if (subOption in subOptions) {
            ...
            if ("keys" in matchingSubOption) {
                validateSubOptions(subOption, optionValue[subOption], matchingSubOption.keys);
            }
        }
        else {
            throw new InvalidSubOptionNameError_1.default(optionName, subOption);
