...
exports.isValidHTMLElement = exports.isValidHierarchy = void 0;
...
const isValidHierarchy = (hierarchy) => hierarchy != null && typeof hierarchy === 'object' && !Array.isArray(hierarchy);
exports.isValidHierarchy = isValidHierarchy;
const isValidHTMLElement = (target) => target != null && target instanceof HTMLElement;
...
const validateReference = (hierarchy, references) => references.filter((ref) => ref.hierarchy.ref === hierarchy.from.ref && (0, idfier_1.getTemmeId)(hierarchy) !== (0, idfier_1.getTemmeId)(ref.hierarchy))[0] != null;
        ...
        if (templates.length > 0) {
            templates.forEach((template) => {
                if (template.from.ref.length > 0) {
                    const referencedElement = references.filter((ref) => ref.hierarchy.ref === template.from.ref)[0], isTemplate = (0, idfier_1.getTemmeId)(referencedElement.hierarchy).length === 4;
                    if (isTemplate === false) {
...
function validateSubOptions(optionName, optionValue, subOptions) {
    for (const subOption in optionValue) {
        const matchingSubOption = (0, options_1.getSubOptions)(optionName).filter((subOptions) => subOptions.label === subOption)[0], subOptionValue = optionValue[subOption];
